...
#import "GBPhotoGroupView.h"
#import "CALayer+GBUtil.h"
#import "UIImage+ImageExtension.h"
#import "UIView+PPAdd.h"
//#import "UIImageView+WebCache.h"

#define kPadding 20
#define kHiColor HEXCOLOR(0x2dd6b8)
...
- (id)copyWithZone:(NSZone *)zone {
    GBPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
- (void)setItem:(GBPhotoGroupItem *)item {
    ...
    //2019 [_imageView sd_cancelCurrentImageLoad];
...
@interface GBPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation GBPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((GBPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
...
//    }
    ...
    GBPhotoGroupItem *item = _groupItems[self.currentPage];
...
//        }
    ...
    if (item.thumbClippedToTop) {
        ...
        if (_selectedIndex != -1) {///凡是跟_selectedIndex有关的代码都是新加的
            cell.imageView.frame = CGRectMake(self.width/2, self.height/2, 1, 2);
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    GBPhotoGroupItem *item = _groupItems[currentPage];
        ...
        if (i >= 0 && i < self.groupItems.count) {
            ...
            if (!cell) {
                ...
                if (_isPresented) {
                    ...
                    if ([self.delegate respondsToSelector:@selector(loadImage:inCell:)]) {
                        [self.delegate loadImage:cell.item.largeImageURL.absoluteString inCell:cell];
...
- (void)longPress {
    ...
    UIViewController *toVC = [self viewControllerForView:self.toContainerView];
    if (!toVC) toVC = [self viewControllerForView:self];
